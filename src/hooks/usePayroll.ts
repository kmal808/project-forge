...
export function usePayroll() {
      ...
      const transformedCrews: CrewPayroll[] = (crewsData || []).map((crew) => ({
        ...
        employees: crew.employees.map((emp) => ({
          ...
          entries: emp.payroll_entries.map((entry) => ({
            ...
            amounts: Array(7).fill(entry.amount / 7), // Distribute amount across days
            date: entry.date,
          })),
        })),
      }));
  ...
  const deleteEmployee = async (employeeId: string) => {
    try {
      ...
      setCrews((prev) =>
        prev.map((crew) => ({
          ...crew,
          employees: crew.employees.filter((emp) => emp.employeeId !== employeeId),
        }))
      );
  ...
  const updateEmployeeEntries = async (
    ...
    try {
        ...
        const { error: insertError } = await supabase
          ...
          .insert(
            entries.map((entry) => ({
              ...
              amount: entry.amounts.reduce((sum, amount) => sum + amount, 0), // Sum daily amounts
              date: entry.date,
              user_id: userData.user.id,
            }))
          );
      ...
      const message = err instanceof Error ? err.message : 'Failed to update payroll entries';
      toast.error(message);
      throw err;
    }
  };
