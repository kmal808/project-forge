		...
		crew: [
			{
				...
				value:
					crews
						.find((c) => c.employees.some((e) => e.employeeId === user?.id))
						?.employees.find((e) => e.employeeId === user?.id)?.entries
						.length || 0,
				icon: Clipboard,
				color: 'bg-blue-500',
				link: '/crews',
							...
							{metrics.map((metric, index) => {
								...
								return (
									<Link
										...
										<div
											className={`rounded-lg ${metric.color} p-2 text-white`}>
