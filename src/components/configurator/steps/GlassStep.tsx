...
import { Sun, Snowflake, Shield, Waves } from 'lucide-react';
...
const glassOptions = [
  {
    id: 'clear',
    name: 'Clear Glass',
    description: 'Maximum visibility and light transmission',
    icon: Sun,
    features: ['High visibility', 'Maximum natural light', 'Basic insulation'],
  },
  {
    id: 'low-e',
    name: 'Low-E Glass',
    description: 'Energy-efficient coating that reflects heat',
    icon: Snowflake,
...
export function GlassStep({ selectedGlass, onComplete }: GlassStepProps) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-900">Select Glass Type</h2>
            ...
            >
              <div className="mb-4 flex items-center justify-between">
                <Icon className={`h-8 w-8 ${
                  selectedGlass === glass.id ? 'text-indigo-600' : 'text-gray-400'
                }`} />
                ...
                {glass.features.map((feature, index) => (
                  <li key={index} className="flex items-center text-sm text-gray-600">
                    <span className={`mr-2 h-1.5 w-1.5 rounded-full ${
                      selectedGlass === glass.id ? 'bg-indigo-600' : 'bg-gray-400'
                    }`} />
                    {feature}
                  </li>
                ))}
