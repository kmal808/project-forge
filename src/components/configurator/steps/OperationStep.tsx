...
const operationOptions: Record<ProductType, Array<{ id: string; name: string; description: string; icon: React.ElementType }>> = {
  'windows': [
    {
      id: 'sliding-left',
  ...
  'siding': [
    {
      id: 'horizontal',
      name: 'Horizontal',
      description: 'Traditional horizontal installation',
      icon: ArrowLeftRight
    },
    {
      id: 'vertical',
      name: 'Vertical',
      description: 'Modern vertical installation',
      icon: ArrowUpDown
    }
  ],
  'security-doors': [
...
export function OperationStep({ productType, selectedOperation, onComplete }: OperationStepProps) {
  const operations = operationOptions[productType] || [];

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-900">Select Operation Style</h2>
          ...
          return (
            ...
            >
              <Icon className={`h-12 w-12 ${
                selectedOperation === operation.id ? 'text-indigo-600' : 'text-gray-400'
              }`} />
              <h3 className="mt-4 font-medium text-gray-900">{operation.name}</h3>
