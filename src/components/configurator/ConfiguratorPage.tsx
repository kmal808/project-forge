...
import type { ConfiguratorStep, ConfiguratorItem, ProductType } from '../../types';
import { ShoppingCart } from 'lucide-react';

export function ConfiguratorPage() {
  const [currentStep, setCurrentStep] = React.useState<ConfiguratorStep>('product-type');
  const [cart, setCart] = React.useState<ConfiguratorItem[]>([]);
  const [currentItem, setCurrentItem] = React.useState<Partial<ConfiguratorItem>>({});
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Product Configurator</h1>
            <p className="mt-2 text-sm text-gray-600">
              Build and customize your perfect solution
            </p>
          </div>
          <button
              ...
              <ErrorBoundary>
                {currentStep === 'product-type' && (
                  <ProductTypeStep
                    ...
                    onComplete={(type) => handleStepComplete({ productType: type })}
                  />
                )}
                {currentStep === 'series' && (
                  <SeriesStep
                ...
                {currentStep === 'operation' && (
                  <OperationStep
                    ...
                    onComplete={(operation) => handleStepComplete({ operation })}
                  />
                )}
                {currentStep === 'color' && (
                  <ColorStep
                    selectedColor={currentItem.frameColor}
                    onComplete={(color) => handleStepComplete({ frameColor: color })}
                  />
                )}
                {currentStep === 'glass' && (
                  <GlassStep
                    selectedGlass={currentItem.glassType}
                    onComplete={(glass) => handleStepComplete({ glassType: glass })}
                  />
                )}
                ...
                {currentStep === 'screen' && (
                  <ScreenStep
                    ...
                    onComplete={(screen) => handleStepComplete({ screenType: screen })}
                  />
                )}
                {currentStep === 'review' && (
                  <ReviewStep
