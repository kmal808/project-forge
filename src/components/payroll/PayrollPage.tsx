...
export function PayrollPage() {
  const { crews, addCrew, updateCrew, deleteCrew, deleteEmployee, updateEmployeeEntries, isLoading } = usePayroll();
  ...
  const [draftEntries, setDraftEntries] = React.useState<Record<string, PayrollEntry[]>>({});
  ...
  const handleUpdateEmployeeName = async (crewId: string, employeeId: string, newName: string) => {
    const crew = crews.find((c) => c.crewId === crewId);
    if (crew) {
  ...
  const handleAddEntry = (crewId: string, employeeId: string, entry: PayrollEntry) => {
    const key = `${crewId}-${employeeId}`;
    setDraftEntries((prev) => ({
      ...prev,
      [key]: [...(prev[key] || []), entry],
  ...
  const handleRemoveEntry = (crewId: string, employeeId: string, index: number) => {
    const key = `${crewId}-${employeeId}`;
    setDraftEntries((prev) => {
  ...
  const handleSubmitEntries = async (crewId: string, employeeId: string, entries: PayrollEntry[]) => {
            ...
            {crew.employees.map((employee) => {
              ...
              return (
                <EmployeePayrollTable
                  ...
                  onNameChange={(name) => handleUpdateEmployeeName(crew.crewId, employee.employeeId, name)}
                  ...
                  onSubmitEntries={(entries) =>
                    handleSubmitEntries(crew.crewId, employee.employeeId, entries)
                  }
                />
              );
            })}
