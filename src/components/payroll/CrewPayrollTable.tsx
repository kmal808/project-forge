...
export function EmployeePayrollTable({
	...
	const weekTotal = entries.reduce(
		(sum, entry) =>
			sum + entry.amounts.reduce((daySum, amount) => daySum + amount, 0),
		0
	)
		...
		<div className='mb-8'>
					...
					{isEditingName ? (
						<div className='flex items-center gap-2'>
							<input
								...
								className='rounded-md border-gray-300 text-secondary text-lg font-medium min-w-[200px]'
								autoFocus
								onKeyDown={(e) => {
									if (e.key === 'Enter') {
										handleNameSave()
							...
							<button
								...
								className='rounded-full p-1 text-primary hover:accent-hover hover:text-accent'
			...
			<div className='overflow-x-auto'>
				<table className='min-w-full divide-y divide-gray-200'>
					<thead>
						<tr>
							<th className='px-3 py-2 text-left text-sm font-semibold text-primary min-w-[200px]'>
								Job
							</th>
							<th className='px-3 py-2 text-left text-sm font-semibold text-primary min-w-[120px]'>
								#
							</th>
							{days.map((day) => (
								<th
									key={day}
									className='px-3 py-2 text-center text-sm font-semibold text-primary min-w-[100px]'>
									{day}
								</th>
							))}
							<th className='px-3 py-2 text-center text-sm font-semibold text-primary'>
								Actions
							</th>
						</tr>
					</thead>
					<tbody className='divide-y divide-gray-200'>
							...
							<tr key={index} className='hover:bg-brand-orange'>
								<td className='whitespace-nowrap px-3 py-2'>
									<input
										...
										className='w-full rounded-sm border-gray-300 text-secondary text-sm'
									/>
								</td>
								<td className='whitespace-nowrap px-3 py-2'>
									<input
								...
								{days.map((day, dayIndex) => (
									<td
										key={day}
										className='whitespace-nowrap text-secondary px-3 py-2'>
										<input
											type='number'
											...
											className='w-full rounded-sm border-gray-300 text-right text-secondary text-sm'
										/>
									</td>
								))}
						...
						{showEntryForm && (
							<tr className='bg-secondary/5'>
								<td className='whitespace-nowrap text-secondary px-3 py-2'>
									<input
										type='text'
										...
										className='w-full rounded-sm border-gray-300 text-primary text-sm'
								...
								{days.map((day, dayIndex) => (
									<td
										...
										<input
											...
											className='w-full rounded-sm border-gray-300 text-right text-primary text-sm'
											placeholder='0.00'
										/>
									</td>
								))}
